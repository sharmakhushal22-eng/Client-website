...
**Demo booking** → `demo_bookings`. `provider_booking_uid` is `unique`, so a
retried or duplicated scheduler webhook updates the existing row instead of
creating a second. `advance_lead_on_booking()` moves the linked lead to `Demo booked` — but only
when the booking carries a `lead_id` and the lead is still `New` or
