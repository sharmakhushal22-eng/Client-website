...
function channels(pct: number) {
  ...
  return {
    g: Math.round(255 - GREEN_DROP * t),
    b: Math.round(255 - BLUE_DROP * t),
  };
}
...
export function ComfortModes() {
  ...
  return (
    <section
      className="bg-canvas py-12 sm:py-14 lg:py-16"
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-xl bg-surface p-7 ring-1 ring-ink-200 sm:p-8">
            ...
            <ul className="mt-5 space-y-2.5 border-t border-ink-200 pt-5">
              {[
                /* Framed explicitly as the APP throughout. These bullets used
                   to read as though they described this page too, which was
                   true when the site carried a live toggle and is not now. */
                [
                  "Three states, not two",
                  "In the app: light, dark, or follow the operating system — because that is a real preference a two-way switch overrides.",
                ],
                [
                  "No flash on load",
                  "The app applies the theme before first paint, so a user on dark never gets a white screen for a frame.",
                ],
                [
                  "Every colour moves together",
                  "One attribute on the document, and every surface, border and state colour in the app is a variable keyed off it.",
                ],
              ].map(([t, d]) => (
                <li key={t} className="flex items-start gap-2.5">
                  ...
                  <span>
                    ...
                    <span className="mt-0.5 block text-[0.82rem] leading-relaxed text-ink-600">
                      {d}
                    </span>
                  </span>
                </li>
            ...
            {/* This was a live toggle that restyled the marketing site as
                proof of the feature. The site is light-only now, so the
                honest version SHOWS the two grounds side by side and says
                where the switch actually lives — rather than implying a
                control the visitor cannot use here. */}
            <div className="mt-6 overflow-hidden rounded-xl ring-1 ring-ink-200">
              <div className="grid grid-cols-2">
                <div className="bg-surface p-4">
                  <span className="text-[0.7rem] font-bold uppercase tracking-[0.12em] text-ink-500">
                    Day
                  </span>
                  <p className="mt-2 text-lg font-bold leading-none text-ink-900">
                    Aa
                  </p>
                  <span className="mt-3 block h-1.5 w-10 rounded-full bg-brand-600" />
                  <span className="mt-1.5 block h-1.5 w-14 rounded-full bg-ink-200" />
                </div>
                <div className="bg-[#111827] p-4">
                  <span className="text-[0.7rem] font-bold uppercase tracking-[0.12em] text-on-dark-faint">
                    Night
                  </span>
                  <p className="mt-2 text-lg font-bold leading-none text-white">
                    Aa
                  </p>
                  <span className="mt-3 block h-1.5 w-10 rounded-full bg-brand-400" />
                  <span className="mt-1.5 block h-1.5 w-14 rounded-full bg-white/20" />
                </div>
              </div>
              <p className="bg-brand-50 px-4 py-3 text-[0.8rem] leading-snug text-ink-700">
                Both ship in the product, set per user, and the choice follows
                them across devices. This site stays in day mode.
              </p>
            </div>
          </div>
          ...
          <div className="rounded-xl bg-surface p-7 ring-1 ring-ink-200 sm:p-8">
            ...
            <div className="mt-6 rounded-md bg-ink-50 p-5 ring-1 ring-ink-200">
              ...
              <p className="mt-3 text-[0.72rem] leading-relaxed text-ink-600">
                At full strength the blue channel is emitted at{" "}
                {channels(100).b}/255 — a 64% cut — while primary text still
                measures 11.44:1 and muted text 6.25:1. Both stay clear of the
                4.5:1 the rest of the product is held to, which is why the
                slider is allowed to run all the way.
              </p>
            </div>
